...
function FinancialReportPanel({ normalRequests, tenkhahRequests, projects }) {
  ...
  return <Card className="min-h-[420px] rounded-2xl border-neutral-200 p-4 shadow-none dark:border-neutral-800"><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><span><span className="block text-sm font-bold">گزارش همه درخواست‌ها</span><span className="mt-1 block text-[11px] text-neutral-500 dark:text-neutral-400">درخواست‌های پرداخت و تنخواه، بدون محدودیت کارتابل یا دخالت کاربر</span></span><input value={query} onChange={(event) => setQuery(event.target.value)} className="h-10 w-full rounded-xl border border-black/10 bg-white px-3 text-sm outline-none transition placeholder:text-neutral-400 focus:border-indigo-400 dark:border-white/10 dark:bg-neutral-900 sm:w-80" placeholder="جست‌وجو در همه درخواست‌ها..." /></div><div className="overflow-hidden rounded-2xl border border-black/10 bg-white text-black dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100"><div className="max-h-[55vh] overflow-auto"><table className="w-full min-w-[900px] table-fixed text-sm [&_th]:whitespace-nowrap [&_th]:text-center [&_td]:min-w-0 [&_td]:text-center"><colgroup><col style={{ width: 90 }} /><col style={{ width: 100 }} /><col style={{ width: 205 }} /><col /><col style={{ width: 165 }} /><col style={{ width: 130 }} /><col style={{ width: 140 }} /></colgroup><thead><tr className="border-b border-neutral-300 bg-neutral-200 text-black dark:border-neutral-700 dark:bg-white/10 dark:text-neutral-100"><th className="sticky top-0 z-30 bg-neutral-200 px-3 py-2 text-[14px] font-semibold dark:bg-neutral-800">نوع</th><th className="sticky top-0 z-30 bg-neutral-200 px-3 py-2 text-[14px] font-semibold dark:bg-neutral-800">شماره</th><th className="sticky top-0 z-30 bg-neutral-200 px-3 py-2 text-[14px] font-semibold dark:bg-neutral-800">تاریخ</th><th className="sticky top-0 z-30 bg-neutral-200 px-3 py-2 text-right text-[14px] font-semibold dark:bg-neutral-800">پروژه</th><th className="sticky top-0 z-30 bg-neutral-200 px-3 py-2 text-right text-[14px] font-semibold dark:bg-neutral-800">موضوع</th><th className="sticky top-0 z-30 bg-neutral-200 px-3 py-2 text-[14px] font-semibold dark:bg-neutral-800">مبلغ</th><th className="sticky top-0 z-30 bg-neutral-200 px-3 py-2 text-[14px] font-semibold dark:bg-neutral-800">درخواست‌کننده</th><th className="sticky top-0 z-30 bg-neutral-200 px-3 py-2 text-[14px] font-semibold dark:bg-neutral-800">وضعیت</th></tr></thead><tbody className="text-[13px] text-black [&>tr]:h-10 dark:text-neutral-100">{filtered.length ? filtered.map((item) => <tr key={item.key} className={`border-b border-neutral-300 ${item.kind === "tenkhah" ? "bg-violet-50/90 dark:bg-violet-500/[0.12]" : "bg-black/[0.02] dark:bg-white/5"} dark:border-neutral-700`}><td className="px-3"><span className={`rounded-md px-2 py-1 text-[11px] font-medium ${item.kind === "tenkhah" ? "bg-violet-100 text-violet-800 dark:bg-violet-500/20 dark:text-violet-200" : "bg-sky-100 text-sky-800 dark:bg-sky-500/20 dark:text-sky-200"}`}>{item.kind === "tenkhah" ? "تنخواه" : "پرداخت"}</span></td><td className="px-3 tabular-nums">{item.serial || "—"}</td><td className="px-3 tabular-nums">{item.date || "—"}</td><td className="truncate px-3 text-right">{item.project || "—"}</td><td className="truncate px-3 text-right">{item.title || "—"}</td><td className="px-3 whitespace-nowrap tabular-nums">{faNumber(item.amount)} {item.currency}</td><td className="truncate px-3">{item.requester || "—"}</td><td className="px-3"><span className="whitespace-nowrap text-neutral-600 dark:text-neutral-300">{reportStatusLabel(item.status)}</span></td></tr>) : <tr><td colSpan="8" className="h-28 px-3 text-center text-sm text-neutral-500 dark:text-neutral-400">موردی یافت نشد.</td></tr>}</tbody></table></div></div></Card>;
}
