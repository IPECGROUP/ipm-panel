...
export default function EstimatesPage() {
  ...
  useEffect(() => {
    ...
    if (!active) {
      setAllowed(false);
      setDenyMsg("شما سطح دسترسی لازم را ندارید.");
      return;
    }
  ...
  }, [active, tabs, tabAccess, tabsReady]);
  ...
  const [openInfoFor, setOpenInfoFor] = useState(null);
  const infoAnchorsRef = useRef({});
  const [infoPos, setInfoPos] = useState({ top: 0, left: 0, width: 320 });

  const placeInfoNear = useCallback((code) => {
    const margin = 8;
    const target = infoAnchorsRef.current[code];
    if (!target) return;
  ...
  useEffect(() => {
    if (!openInfoFor) return;
    placeInfoNear(openInfoFor);
    const onResize = () => placeInfoNear(openInfoFor);
  ...
  }, [openInfoFor, placeInfoNear]);

  useEffect(() => {
    const close = () => setOpenInfoFor(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      setErr("");
      if (!tabsReady) return;
      if (!allowed || !tabs.find((t) => t.id === active)) {
        setRows([]);
        return;
      }
  ...
  const exportExcel = () => {
    ...
    const bodyHtml = rowsData
      .map((r, i) => {
        const finalPreview = finalPreviewOf(r);
        const newAmount = sumMonths(r);
        return `
        <tr>
          <td>${buildCell(toFaDigits(i + 1))}</td>
          <td>${buildCell(renderCode(r.code))}</td>
          <td>${buildCell(r.name || "—")}</td>
          <td>${buildCell(r.desc || "—")}</td>
          <td>${buildCell(toFaDigits(formatMoney(r.baseAmount || 0)))}</td>
          <td>${buildCell(toFaDigits(formatMoney(finalPreview || 0)))}</td>
          <td>${buildCell(toFaDigits(formatMoney(newAmount || 0)))}</td>
        </tr>
      `;
      })
      .join("");
