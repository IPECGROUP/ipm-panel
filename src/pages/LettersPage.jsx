...
const resolveFileUrl = (u) => {
  ...
  // âœ… ÙÙ‚Ø· Ø§ÛŒÙ† Ø¯Ùˆ Ù†ÙØ± + Ù†Ù‚Ø´ admin Ø¯Ø³ØªØ±Ø³ÛŒ Ù…Ø­Ø±Ù…Ø§Ù†Ù‡ Ø¯Ø§Ø±Ù†Ø¯
...
const canSeeConfidential = useMemo(() => {
  ...
  const role = String(user?.role || "").trim().toLowerCase(); // Ø§Ú¯Ø± role Ø¯Ø§Ø±ÛŒ
...
// Ø§Ú¯Ø± Ø¬Ø§Ù‡Ø§ÛŒ Ø¯ÛŒÚ¯Ù‡ Ø§Ø² isAdmin Ø§Ø³ØªÙØ§Ø¯Ù‡ Ù…ÛŒâ€ŒÚ©Ù†ÛŒ:
...
lastSavedFormTagsRef.current = JSON.stringify(ids);
  ...
  // âœ… Ù‡Ù… UI Ù‡Ø± Ø³Ù‡ ØªØ¨ ÛŒÚ©ÛŒ Ø´ÙˆØ¯
...
const setFormTagsAllAndPersist = async (ids) => {
  ...
  // âœ… UI: Ø¨Ø±Ú†Ø³Ø¨â€ŒÙ‡Ø§ Ø¯Ø± Ù‡Ø± Ø³Ù‡ ØªØ¨ ÛŒÚ©ÛŒ
  setIncomingTagIds(next);
  setOutgoingTagIds(next);
  setInternalTagIds(next);

  // âœ… Ø§Ú¯Ø± Ù‡ÛŒÚ† ØªØºÛŒÛŒØ±ÛŒ Ù†Ú©Ø±Ø¯Ù‡ØŒ Ø§ØµÙ„Ø§Ù‹ POST Ù†Ø²Ù†
...
const tagsForFormScope = useMemo(() => {
  ...
  // Ù¾Ø§ÛŒÙ‡ Ù†Ù…Ø§ÛŒØ´: Ù‡Ù…ÙˆÙ† latestTags (Ø«Ø§Ø¨Øª)
  const base = Array.isArray(latestTags) ? latestTags : [];

  // Ø§Ú¯Ø± ØªÚ¯ÛŒ Ø§Ù†ØªØ®Ø§Ø¨ Ø´Ø¯Ù‡ ÙˆÙ„ÛŒ ØªÙˆ latest Ù†ÛŒØ³ØªØŒ Ø¢Ø®Ø± Ù„ÛŒØ³Øª Ø§Ø¶Ø§ÙÙ‡ Ú©Ù† (Ø¨Ø¯ÙˆÙ† Ø¯Ø³ØªÚ©Ø§Ø±ÛŒ ØªØ±ØªÛŒØ¨ base)
...
const isImageUrl = (url, name = "") =>
  ...
  return arr.filter((l) => {
    // âœ… ÙÙ‚Ø· Ø§Ø¯Ù…ÛŒÙ† Ù…Ø­Ø±Ù…Ø§Ù†Ù‡â€ŒÙ‡Ø§ Ø±Ø§ Ø¨Ø¨ÛŒÙ†Ø¯
    const isConf = isConfidentialLetter(l); // Ù‡Ù…ÙˆÙ†ÛŒ Ú©Ù‡ Ø®ÙˆØ¯Øª Ø¯Ø§Ø±ÛŒ
  ...
  filterFromDate,
  filterToDate,
  canSeeConfidential, // âœ… Ø§Ø¶Ø§ÙÙ‡ Ø´Ø¯
]);
...
// Ø³Ø§Ø²Ú¯Ø§Ø±ÛŒ Ø¨Ø§ Ø¯ÛŒØªØ§Ù‡Ø§ÛŒ Ù‚Ø¯ÛŒÙ…ÛŒ Ø´Ù…Ø§ Ú©Ù‡ category="project" Ø¨ÙˆØ¯Ù‡
    ...
    // Ø·Ø¨Ù‚Ù‡ Ø¨Ù†Ø¯ÛŒ (Ø§Ú¯Ø± Ø§Ø² Ø¨Ú©â€ŒØ§Ù†Ø¯ Ø§ÙˆÙ…Ø¯ØŒ ÙˆÚ¯Ø±Ù†Ù‡ Ù¾ÛŒØ´â€ŒÙØ±Ø¶)
    const rawClass =
      String(l?.classification ?? l?.doc_classification ?? l?.confidentiality ?? "").trim();
    setClassification(rawClass || "Ø¹Ø§Ø¯ÛŒ");
...
const payload = {
  ...
  classification:
    kind === "incoming" ? (incomingForm.classification || "Ø¹Ø§Ø¯ÛŒ")
    : "Ø¹Ø§Ø¯ÛŒ",
...
const deleteAllLetters = async () => {
  ...
  const InfoRow = ({ label, value }) => (
    <div className="grid grid-cols-12 gap-2 py-2">
      ...
      <div className={"col-span-8 text-sm " + (theme === "dark" ? "text-white" : "text-neutral-900")}>{value || "â€”"}</div>
    </div>
  );
...
const viewKind = useMemo(() => (viewLetter ? letterKindOf(viewLetter) : ""), [viewLetter]); // eslint-disable-line react-hooks/exhaustive-deps
const isViewIncoming = viewKind === "incoming";
        ...
        >
            ...
            <td className={"!pl-6 !pr-3 " + divider}>
              <div className="w-full flex items-center justify-start gap-2 pl-3">
                <button type="button" onClick={() => openView(l)} className={iconBtnCls} aria-label="Ù†Ù…Ø§ÛŒØ´" title="Ù†Ù…Ø§ÛŒØ´">
                      ...
                      <div className="lg:w-[56%] h-full overflow-auto p-4">
                        <div className={"rounded-2xl border overflow-hidden " + (theme === "dark" ? "border-white/10 bg-white/5" : "border-black/10 bg-black/[0.02]")}>
                          ...
                          <div className="px-4 divide-y divide-black/10 dark:divide-white/10">
                            ...
                            />
                            <InfoRow
                              label={isViewIncoming ? "کلاس سند" : "دسته بندی"}
                                    ...
                                    ? (() => {
                                        ...
                                        return (
                                          <div className="flex flex-wrap gap-2">
                                            {clean.map((sid) => {
                                              const t = tagById.get(sid);
                                              const label = t ? tagLabelOf(t) : `برچسب (${toFaDigits(sid)})`;
                                              return (
                                                <span key={sid} className={chipCls}>
                                                  {label}
                                                </span>
                                              );
                                            })}
                                  ...
                                  viewLetter
                                    ? (() => {
                                        ...
                                        const labels = ids
                                          ...
                                          .filter(Boolean)
                                          .map((sid) => {
                                            const it = letterById.get(sid);
                                            return it ? String(letterNoOf(it) || sid) : sid;
                                          });
                                        return labels.join("، ");
                                      })()
                                    : ""
                            ...
                            <InfoRow
                              label={isViewIncoming ? "نامه های مرتبط" : "پیرو"}
                              value={
                                viewLetter
                                  ? (() => {
                                      ...
                                      return (
                                        <div className="flex flex-wrap gap-2">
                                          {clean.map((sid) => {
                                            const it = letterById.get(sid);
                                            const no = it ? String(letterNoOf(it) || sid) : sid;
                                            const txt = toFaDigits(no);
