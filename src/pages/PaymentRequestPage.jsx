...
export default function PaymentRequestPage() {
  ...
  const removeAttachment = (index) => {
    setForm((old) => ({ ...old, attachments: old.attachments.filter((_, currentIndex) => currentIndex !== index) }));
  };
      ...
      <div className="p-2.5 sm:p-3 md:p-4">
        <div className="mb-4 flex min-w-0 items-center justify-between gap-2 border-b border-black/[0.07] px-0.5 pb-3 dark:border-white/10 sm:mb-5 sm:gap-3 sm:pb-4">
          ...
          <div className="flex items-center gap-2">
            <button type="button" onClick={() => { if (!showForm) setForm(emptyForm()); setShowForm((old) => !old); setError(""); setSuccess(""); }} className="flex h-10 w-10 items-center justify-center rounded-xl ring-1 ring-black/15 transition hover:bg-black/5 dark:ring-neutral-800 dark:hover:bg-white/10" title={showForm ? "نمایش لیست" : "افزودن درخواست"}>
        ...
        {uploadOpen && <PaymentUploadModal files={form.attachments} uploading={uploading} onUpload={uploadFiles} onRemove={removeAttachment} onClose={() => setUploadOpen(false)} />}
          ...
          <div className="relative hidden max-h-[55vh] overflow-y-auto overflow-x-hidden pb-0 md:block" dir="ltr">
            ...
            <tbody className="text-[13px] text-black [&>tr]:h-9 [&>tr>td]:!py-0 dark:text-neutral-100">
              {loading ? <tr><td colSpan={9} className="py-8 text-black/60 dark:text-neutral-400">در حال دریافت...</td></tr> : pageItems.length === 0 ? <tr><td colSpan={9} className="py-8 text-black/60 dark:text-neutral-400">هنوز درخواستی ثبت نشده است.</td></tr> : pageItems.map((item) => <tr key={item.id} onClick={() => item.requestType === "tenkhah" ? setSelectedTenkhah(item) : openPreview(item)} className={`group cursor-pointer transition-colors ${item.requestType === "tenkhah" ? "bg-violet-50/90 hover:bg-violet-100/80 dark:bg-violet-500/[0.12] dark:hover:bg-violet-500/[0.18]" : "bg-black/[0.02] hover:bg-black/[0.04] dark:bg-white/5 dark:hover:bg-white/10"}`}>
                ...
                <td className="border-b border-neutral-300 px-3 dark:border-neutral-700">{item.requestType === "tenkhah" ? <button type="button" onClick={(event) => { event.stopPropagation(); setSelectedTenkhah(item); }} className="mx-auto inline-flex items-center text-[13px] font-normal text-neutral-900 underline-offset-4 transition hover:underline dark:text-neutral-100" title="نمایش درخواست">{toFa(item.serial)}</button> : <button type="button" onClick={(event) => { event.stopPropagation(); openPreview(item); }} className="mx-auto inline-flex items-center justify-center text-[13px] font-normal underline-offset-4 transition hover:underline" title="نمایش درخواست">{toFa(displayPaymentSerial(item, projects))}</button>}</td>
    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-3 sm:p-5" dir="rtl">
      ...
      <section className="relative flex max-h-[min(86vh,760px)] w-[min(920px,calc(100vw-24px))] flex-col overflow-hidden rounded-2xl border border-black/10 bg-white text-neutral-900 shadow-2xl dark:border-white/10 dark:bg-neutral-900 dark:text-white">
        <header className="flex items-center justify-between gap-3 border-b border-black/10 px-4 py-3 dark:border-white/10">
          <div className="text-base font-bold">انتخاب کد بودجه</div>
        ...
        <div className="min-h-40 flex-1 overflow-y-auto">
          {displayRows.length ? displayRows.map((row) => <div key={row.code} className={`group grid grid-cols-[42px_150px_minmax(0,1fr)] items-center border-b border-black/[0.07] px-3 py-1.5 transition last:border-b-0 hover:bg-black/[0.035] dark:border-white/[0.08] dark:hover:bg-white/[0.05] ${selectedCode === row.code ? "bg-sky-50 dark:bg-sky-500/10" : ""}`}>
            ...
            <button type="button" onClick={() => onSelect(row.code)} className={`min-w-0 py-2 text-right font-sans text-sm tabular-nums ${row.hasChildren ? "font-bold" : "font-medium"}`} style={{ paddingRight: `${row.depth * 26}px` }} dir="ltr" title={`انتخاب ${row.code}`}>{toFa(row.code)}</button>
...
function PaymentUploadModal({ files, uploading, onUpload, onRemove, onClose }) {
            ...
            <div>
              ...
              <div className="overflow-hidden rounded-2xl border border-black/10 dark:border-white/10">
                ...
                <div className="min-h-16 p-3 text-center text-sm text-neutral-500 dark:text-neutral-400">
                  {list.length ? <div className="space-y-2 text-right">{list.map((file, index) => <div key={file.id || file.serverId || file.url || index} className="flex items-center gap-2 rounded-lg bg-black/[0.03] px-2 py-1.5 text-xs text-neutral-700 dark:bg-white/5 dark:text-neutral-200"><span className="min-w-0 flex-1 truncate">{file.name || `فایل ${toFa(index + 1)}`}</span><button type="button" onClick={() => onRemove(index)} className="grid h-6 w-6 shrink-0 place-items-center rounded-md text-base transition hover:bg-black/10 dark:hover:bg-white/15" title="حذف فایل" aria-label={`حذف ${file.name || `فایل ${toFa(index + 1)}`}`}>×</button></div>)}</div> : "فایلی انتخاب نشده است."}
                </div>
                <div className="p-3">
                  <div
...
function TenkhahDetailCards({ details }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-neutral-900">
    ...
    <div>
      {detailRows.map((row, rowIndex) => <div key={rowIndex} className={`grid grid-cols-1 divide-y divide-black/10 dark:divide-white/10 md:grid-cols-3 md:divide-y-0 md:[&>*+*]:border-r md:[&>*+*]:border-black/20 dark:md:[&>*+*]:border-white/15 ${rowIndex < detailRows.length - 1 ? "border-b border-black/10 dark:border-white/10" : ""}`}>
        {row.map(([label, value]) => <div key={label} className="min-w-0 px-3 py-3 text-xs leading-6">
          <div className="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5">
            <span className="shrink-0 whitespace-nowrap text-neutral-500 dark:text-neutral-400">{label}:</span>
            <span className="min-w-0 break-words font-medium text-neutral-900 dark:text-neutral-100">{value || "—"}</span>
          </div>
        </div>)}
      </div>)}
    </div>
...
function TenkhahFinalPaymentForm({
  currencyTypes, fallbackCurrency, cashPaymentAmount, setCashPaymentAmount,
  cashPaymentCurrency, setCashPaymentCurrency, cashPaymentMethod, setCashPaymentMethod,
  creditPaymentAmount, setCreditPaymentAmount, creditPaymentCurrency, setCreditPaymentCurrency,
  creditPaymentDescription, setCreditPaymentDescription, hasFinalPayment, busy, error, onSubmit,
}) {
  const currencies = Array.from(new Set([fallbackCurrency, ...(currencyTypes || []).map(itemLabel)].filter(Boolean)));
  return <section className="overflow-hidden rounded-2xl border border-black/10 dark:border-white/10">
    ...
    <div className="space-y-4 p-4">
      <div className="rounded-2xl border border-black/10 p-4 dark:border-white/10">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <Field label="مبلغ"><MoneyInput value={cashPaymentAmount} onChange={setCashPaymentAmount} /></Field>
          <Field label="ارز"><select className={inputClass} value={cashPaymentCurrency} onChange={(event) => setCashPaymentCurrency(event.target.value)}>{currencies.map((currency) => <option key={currency} value={currency}>{currency}</option>)}</select></Field>
          <Field label="روش پرداخت"><select className={inputClass} value={cashPaymentMethod} onChange={(event) => setCashPaymentMethod(event.target.value)}><option value="">انتخاب کنید</option><option value="واریز بانکی - فیش">واریز بانکی - فیش</option><option value="چک">چک</option><option value="نقد">نقد</option></select></Field>
        </div>
      </div>
      <div className="rounded-2xl border border-black/10 p-4 dark:border-white/10">
        <div className="mb-3 text-sm font-bold">پرداخت اعتباری</div>
        <div className="grid gap-3 md:grid-cols-2">
          <Field label="مبلغ"><MoneyInput value={creditPaymentAmount} onChange={setCreditPaymentAmount} /></Field>
          <Field label="ارز"><select className={inputClass} value={creditPaymentCurrency} onChange={(event) => setCreditPaymentCurrency(event.target.value)}>{currencies.map((currency) => <option key={currency} value={currency}>{currency}</option>)}</select></Field>
        </div>
        <div className="mt-3"><Field label="شرح پرداخت"><textarea className={`${inputClass} min-h-24 py-3`} value={creditPaymentDescription} onChange={(event) => setCreditPaymentDescription(event.target.value)} /></Field></div>
      </div>
      {error && <div className="text-sm text-red-600">{error}</div>}
      <div className="flex justify-end"><button type="button" disabled={busy || !hasFinalPayment} onClick={onSubmit} className="grid h-10 w-10 place-items-center rounded-xl bg-black text-white disabled:cursor-not-allowed disabled:opacity-50 dark:bg-white dark:text-black" title="ثبت نهایی پرداخت" aria-label="ثبت نهایی پرداخت"><img src="/images/icons/check.svg" alt="" className="h-4 w-4 invert dark:invert-0" /></button></div>
    </div>
  </section>;
}
...
function TenkhahPreviewV4({ item, userId, api, onRefresh, onClose }) {
  ...
  const [cashPaymentCurrency, setCashPaymentCurrency] = useState(item.currency || "ریال");
  const [cashPaymentMethod, setCashPaymentMethod] = useState("واریز بانکی - فیش");
  const [creditPaymentAmount, setCreditPaymentAmount] = useState("");
  const [creditPaymentCurrency, setCreditPaymentCurrency] = useState(item.currency || "ریال");
  const [creditPaymentDescription, setCreditPaymentDescription] = useState("");
  const canAct = (item.canAct === true || Number(item.currentAssigneeUserId) === Number(userId)) && item.status === "pending";
  ...
  const details = [["شماره درخواست", item.serial || item.requestNumber || "—"], ["تاریخ درخواست", toFa(String(item.dateFa || item.requestDate || "—").replaceAll("-", "/"))], ["پروژه", `${item.projectCode || ""}${item.projectName ? ` - ${item.projectName}` : ""}` || "—"], ["درخواست‌کننده", item.createdByName || item.requesterName || "—"], ["ذینفع", item.beneficiaryName || item.beneficiaryUsername || "—"], ["بابت", item.purpose || "—"], ["مبلغ تنخواه", `${toFa(money(item.amount || item.requestedAmount || "0"))} ${item.currencyName || item.currency || "ریال"}`]];
  useEffect(() => {
              ...
              <PreviewSection title="جزئیات درخواست پرداخت" flush>
                ...
                <div className="grid grid-cols-1 divide-y divide-black/10 md:grid-cols-[1.15fr_1fr_1fr] md:divide-y-0 md:[&>*+*]:border-r md:[&>*+*]:border-black/20 dark:md:[&>*+*]:border-white/15 dark:divide-white/10">
                  ...
                  <PreviewRow compact editing={canEditRelatedDocuments} colon leader={!canEditRelatedDocuments} label="اسناد مرتبط" value={canEditRelatedDocuments ? <div className="flex flex-wrap items-center justify-end gap-2">
                    ...
                    {relatedLetters.map((letter) => <button key={letter.id} type="button" onClick={() => setRelatedLetterPreview(letter.letter || { id: letter.id })} className="rounded-lg border border-black/10 px-2 py-1 text-xs font-semibold underline underline-offset-4 transition hover:bg-black/5 dark:border-white/10 dark:hover:bg-white/10" title={letter.subject || "پیش‌نمایش نامه"}>{letter.label}</button>)}
                  </div> : (relatedLetters.length ? <div className="flex flex-wrap justify-end gap-2">{relatedLetters.map((letter) => <button key={letter.id} type="button" onClick={() => setRelatedLetterPreview(letter.letter || { id: letter.id })} className="rounded-lg border border-black/10 px-2 py-1 text-xs font-semibold underline underline-offset-4 transition hover:bg-black/5 dark:border-white/10 dark:hover:bg-white/10" title={letter.subject || "پیش‌نمایش نامه"}>{letter.label}</button>)}</div> : "—")} />
                </div>
...
function ActionOption({ kind = "approve", checked, disabled, onClick, label, children }) {
  ...
  return <div role="button" tabIndex={disabled ? -1 : 0} onClick={() => !disabled && onClick()} onKeyDown={(event) => { if (!disabled && (event.key === "Enter" || event.key === " ")) onClick(); }} className={`min-h-[154px] cursor-pointer rounded-2xl border p-4 text-center transition ${checked ? appearance.selected : "border-black/10 bg-white hover:border-black/20 hover:shadow-sm dark:border-white/10 dark:bg-white/[0.03] dark:hover:border-white/20"} ${disabled ? "cursor-not-allowed opacity-45" : ""}`}>
    <div className={`mx-auto grid h-10 w-10 place-items-center rounded-full text-2xl font-bold ${appearance.iconClass}`}>{kind === "approve" || kind === "reject" || kind === "return" ? <img src={kind === "approve" ? "/images/icons/finishing-check.svg" : kind === "reject" ? "/images/icons/rad.svg" : "/images/icons/bazgasht.svg"} alt="" className="h-5 w-5" /> : appearance.icon}</div>
