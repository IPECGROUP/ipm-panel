    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-3 sm:p-5" dir="rtl">
      ...
      <section className="relative flex max-h-[min(86vh,760px)] w-[min(920px,calc(100vw-24px))] flex-col overflow-hidden rounded-2xl border border-black/10 bg-white text-neutral-900 shadow-2xl dark:border-white/10 dark:bg-neutral-900 dark:text-white">
        ...
        <div className="flex flex-col gap-3 border-b border-black/10 p-3 sm:flex-row dark:border-white/10">
          ...
          <button type="button" onClick={() => setExpandedCodes(allExpanded ? new Set() : new Set(allExpandable))} disabled={!allExpandable.length || Boolean(normalizedQuery)} className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-black/10 text-xl font-semibold transition hover:bg-black/[0.04] disabled:cursor-not-allowed disabled:opacity-40 dark:border-white/10 dark:hover:bg-white/10" title={allExpanded ? "بستن همه" : "باز کردن همه"} aria-label={allExpanded ? "بستن همه" : "باز کردن همه"}>{allExpanded ? "−" : "+"}</button>
        </div>
        <div className="grid grid-cols-[42px_150px_minmax(0,1fr)] border-b border-black/10 bg-neutral-100 px-3 py-2 text-xs font-bold text-neutral-700 dark:border-white/10 dark:bg-white/5 dark:text-neutral-200"><span className="text-center text-base leading-4">+</span><span className="text-right">کد بودجه</span><span className="text-right">نام بودجه</span></div>
