...
export function RankingPanel({ number, title, subtitle, rows = [], emptyText = "داده‌ای برای نمایش وجود ندارد" }) {
  return (
    <Card className="min-h-[250px] rounded-2xl border-neutral-200 p-4 shadow-none dark:border-neutral-800">
      <div className="mb-4 flex items-start justify-between gap-3">
        <span><span className="block text-sm font-bold">باکس {faNumber(number)} · {title}</span><span className="mt-1 block text-[11px] text-neutral-500 dark:text-neutral-400">{subtitle}</span></span>
...
export function AveragePanel({ number, averages }) {
  const items = [["ماه", averages?.month], ["هفته", averages?.week], ["روز", averages?.day]];
  return (
    <Card className="min-h-[250px] rounded-2xl border-neutral-200 p-4 shadow-none dark:border-neutral-800">
      <div className="mb-4"><span className="block text-sm font-bold">باکس {faNumber(number)} · میانگین ثبت اسناد</span><span className="mt-1 block text-[11px] text-neutral-500 dark:text-neutral-400">بر اساس کل دورهٔ ثبت اسناد</span></div>
...
export function ProjectDocumentsPanel({ rows = [], className = "" }) {
  return (
    <Card className={`min-h-[285px] rounded-2xl border-neutral-200 p-4 shadow-none dark:border-neutral-800 ${className}`}>
      <div className="mb-4"><span className="block text-sm font-bold">باکس ۶ · اسناد پروژه‌های فعال</span><span className="mt-1 block text-[11px] text-neutral-500 dark:text-neutral-400">تفکیک اسناد وارده، صادره و داخلی</span></div>
      <div className="max-h-[240px] overflow-auto rounded-xl border border-black/[0.07] dark:border-white/[0.08]"><table className="w-full min-w-[490px] text-right text-xs"><thead className="sticky top-0 bg-neutral-50 text-neutral-500 dark:bg-neutral-800 dark:text-neutral-300"><tr><th className="px-3 py-2.5 font-medium">پروژه</th><th className="px-3 py-2.5 text-center font-medium">وارده</th><th className="px-3 py-2.5 text-center font-medium">صادره</th><th className="px-3 py-2.5 text-center font-medium">داخلی</th><th className="px-3 py-2.5 text-center font-medium">کل</th></tr></thead><tbody>{rows.length ? rows.map((row) => <tr key={row.id} className="border-t border-black/[0.06] dark:border-white/[0.08]"><td className="max-w-[260px] truncate px-3 py-3 font-medium">{row.label}</td><td className="px-3 py-3 text-center tabular-nums">{faNumber(row.incoming)}</td><td className="px-3 py-3 text-center tabular-nums">{faNumber(row.outgoing)}</td><td className="px-3 py-3 text-center tabular-nums">{faNumber(row.internal)}</td><td className="px-3 py-3 text-center font-bold tabular-nums">{faNumber(row.total)}</td></tr>) : <tr><td colSpan="5" className="px-3 py-12 text-center text-neutral-400">پروژهٔ فعالی برای نمایش وجود ندارد</td></tr>}</tbody></table></div>
    </Card>
  );
}
