...
function RevenueEstimatesPage() {
  ...
  const sumRowMonths = useCallback(
    (row) =>
      dynamicMonths.reduce(
        (acc, m) => acc + Number(row.months?.[m.key] || 0),
        0,
      ),
    [dynamicMonths],
  );
  ...
  const totalsByMonth = useMemo(() => {
    ...
    rows.forEach((row) => {
      dynamicMonths.forEach((m) => {
        totals[m.key] += Number(row.months?.[m.key] || 0);
      });
    });
    return totals;
  }, [rows, dynamicMonths]);
  ...
  const totalGrand = useMemo(
    () => rows.reduce((acc, r) => acc + sumRowMonths(r), 0),
    [rows, sumRowMonths],
  );
      ...
      try {
        ...
        const mapped = items.map((item) => {
          ...
          (item.months || []).forEach((m) => {
            if (m && m.key) {
              monthsMap[m.key] = Number(m.amount || 0);
            }
          });
  ...
  // ==== مودال ثبت ردیف جدید (پروژه / مورد) ====
  ...
  const handleProjectSelectChange = (e) => {
    ...
    if (!val) {
      ...
      const name =
        pr?.name || pr?.title || pr?.project_name || pr?.project || '';
      setAddModal((prev) => ({
        ...prev,
        title: name,
      }));
  ...
  const handleAddSave = () => {
    ...
    const newRow = {
      ...
      desc,
      projectId:
        addProjectMode === 'select' && addSelectedProjectId
          ? Number(addSelectedProjectId)
          : null,
      months: {},
    };
  ...
  // ==== مودال نمایش جزئیات ردیف (روی چیپ کلیک شود) ====
  const [viewRowModal, setViewRowModal] = useState({
    open: false,
    row: null,
  });

  const openViewRowModal = (row) => {
    setViewRowModal({ open: true, row });
  };

  const closeViewRowModal = () =>
    setViewRowModal({ open: false, row: null });
  ...
  const handleMonthModalSave = () => {
    ...
    setRows((prev) =>
      prev.map((r) => {
        if (r.id !== monthModal.rowId) return r;
        return {
          ...r,
          months: {
            ...(r.months || {}),
  ...
  // ==== ذخیره‌سازی در بک‌اند: کل ردیف + ماه‌ها ====
    ...
    try {
      const payloadRows = rows.map((r, idx) => {
        ...
        const months = dynamicMonths.map((m) => ({
          ...
          label: m.label,
          amount: Number(r.months?.[m.key] || 0),
        }));
        const total = months.reduce(
          (acc, mm) => acc + (mm.amount || 0),
          0,
        );
  ...
  // مودال پیش‌نمایش هنوز هست ولی دیگه دکمۀ باز کردنش رو فعلاً حذف کردیم
  ...
  const exportExcel = () => {
    ...
    const bodyHtml = rows
      .map((r, i) => {
        const rowTotal = sumRowMonths(r);
        const monthsHtml = dynamicMonths
          .map((m) => {
            const val = Number(r.months?.[m.key] || 0);
            return `<td>${
              val ? buildCell(toFaDigits(formatMoney(val))) : '—'
            }</td>`;
          })
          .join('');
        return `
          <tr>
            ...
            ${monthsHtml}
            <td>${
              rowTotal ? buildCell(toFaDigits(formatMoney(rowTotal))) : '—'
            }</td>
          </tr>
        `;
      })
      ...
      <tr>
        ...
        ${dynamicMonths
          .map((m) => {
            ...
            return `<td>${
              v ? buildCell(toFaDigits(formatMoney(v))) : '—'
            }</td>`;
          })
          .join('')}
        <td>${
          totalGrand
            ? buildCell(toFaDigits(formatMoney(totalGrand)))
            : '—'
        }</td>
      </tr>
    `;

    const noRowsHtml = `<tr><td colspan="${
      2 + dynamicMonths.length + 1
    }">موردی برای نمایش نیست.</td></tr>`;
  ...
  return (
    <>
      <Card>
                ...
                <tbody className="[&_td]:text-black dark:[&_td]:text-neutral-100">
                      ...
                      >
                        ...
                        <TD className="px-2 py-3 text-center whitespace-nowrap">
                          <div className="flex items-center justify-center gap-2">
                            {/* چیپ پروژه: کلیک برای نمایش جزئیات */}
                            <button
                              type="button"
                              onClick={() => openViewRowModal(r)}
                              className="inline-flex items-center gap-1 px-3 py-1 rounded-2xl border border-black/10 bg-black/[0.04] text-[11px] text-black cursor-pointer dark:border-neutral-700 dark:bg:white/10 dark:text-neutral-100"
                              title="مشاهده جزئیات"
                            >
                        ...
                        <TD className="px-3 py-3 whitespace-nowrap text-center border-l border-r border-black/10 dark:border-neutral-700">
                          <span className="inline-flex items-center justify-center gap-1">
                            <span className="ltr">
                              {toFaDigits(formatMoney(rowTotal || 0))}
                            </span>
                            <span>ریال</span>
                          </span>
                        </TD>
                  ...
                  <TR className="border-t border-black/10 bg-black/[0.015] dark:border-neutral-800 dark:bg:white/5">
                    ...
                    {dynamicMonths.map((m) => (
                      <TD
                        key={m.key}
                        className="px-0 py-2 text-center text-black/40 dark:text-neutral-500"
                      >
                        —
                      </TD>
                    ))}
                    <TD className="px-3 py-3 text-center text-black/40 dark:text-neutral-500">
                      —
                    </TD>
                  </TR>
                </tbody>
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    placeholder="0"
                    value={
                      monthModal.value ? toFaDigits(monthModal.value) : ''
                    }
                    onChange={(e) =>
                      handleMonthModalChange(e.target.value)
                    }
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
              ...
              <div className="flex items-center justify-between gap-2 pt-1">
                ...
                <button
                  ...
                  className="h-9 px-5 rounded-xl bg-neutral-900 text-xs text-white hover:bg-neutral-800 dark:bg-white dark:text-neutral-900 dark:hover:bg-neutral-200"
                >
                  ثبت برآورد
                </button>
              </div>
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <div className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
                    یک پروژه / مورد جدید برای برآورد درآمد اضافه کنید.
                  </div>
                </div>
                <button
                  type="button"
        ...
        {/* مودال نمایش جزئیات ردیف (روی چیپ) */}
        {viewRowModal.open && (
          <div className="fixed inset-0 z-40 grid place-items-center px-3">
            <div
              className="absolute inset-0 bg-black/25 dark:bg-neutral-950/55 backdrop-blur-[2px]"
              onClick={closeViewRowModal}
            />
            <div
              className="relative w-full max-w-sm rounded-2xl bg-white text-neutral-900 border border-black/10 shadow-2xl dark:bg-neutral-900 dark:text-neutral-100 dark:border-neutral-800 p-4 space-y-3"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <div className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
                    عنوان و توضیحات ثبت‌شده را مشاهده می‌کنید.
                  </div>
                </div>
                <button
                  type="button"
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  ...
                  <div className="w-full min-h-[72px] rounded-xl px-3 py-2 text-sm bg-black/[0.02] text-black border border-black/10 dark:bg-neutral-800 dark:text-neutral-100 dark:border-neutral-700 whitespace-pre-wrap">
                    {viewRowModal.row?.desc?.trim()
                      ? viewRowModal.row.desc
                      : 'بدون توضیحات'}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-1">
                <button
                  type="button"
                  onClick={closeViewRowModal}
                  className="h-9 px-5 rounded-xl border border-neutral-300 text-xs text-neutral-700 hover:bg-neutral-100 dark:border-neutral-700 dark:text-neutral-100 dark:hover:bg-neutral-800"
                >
                  بستن
                </button>
              </div>
        ...
        {/* مودال پیش‌نمایش (دیگه دکمه‌ای برای باز کردنش نداریم، ولی نگه‌داشتیم اگر بعداً بخوای برش گردونیم) */}
        {showModal && (
                  ...
                  <table className="w-full table-fixed text-[11px] md:text-xs text-center [&_th]:text-center [&_td]:text-center">
                    <thead className="bg-black/5 text-black border-b border-black/10 sticky top-0 dark:bg:white/5 dark:text-neutral-100 dark:border-neutral-700">
                      <tr>
                        ...
                        <th className="py-2.5 px-2 w-32 text-center border-l border-r border-black/10 dark:border-neutral-700">
                          جمع
                        </th>
                      </tr>
                    </thead>
                      ...
                      {rows.length === 0 ? (
                        ...
                        <>
                          <tr className="border-t border-b border-black/10 bg-black/[0.04] font-semibold dark:border-neutral-800 dark:bg:white/10">
                            ...
                            <td className="py-2 px-2 w-40 text-center">
                              جمع
                            </td>
                            {dynamicMonths.map((m) => (
                              <td
                          ...
                          {rows.map((r, i) => {
                            ...
                            return (
                              <tr
                                key={r.id}
                                className="border-t border-black/10 dark:border-neutral-800 odd:bg-black/[0.015] dark:odd:bg:white/5"
                              >
                                <td className="py-2 px-2 w-16 text-center">
                                  {toFaDigits(i + 1)}
                                </td>
                                <td className="py-2 px-2 w-40 whitespace-nowrap text-center">
                                  {r.title || '—'}
                                </td>
                                {dynamicMonths.map((m) => {
                                  const val = Number(
